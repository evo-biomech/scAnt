...
**FLIR setup (If using another camera, skip to DSLR Setup)**
...
Camera control is handled by gphoto in Ubuntu, it should be included in the original conda environment install. To test the camera functionality, run the gphoto control file:

```bash
cd scant/GUI
python camera_control_gphoto.py
```
All being well, you should see the camera info displayed in the terminal and 5 images at different ISO values will be captured.
...
A number of open source tools are used for processing the RAW images captured by the scanner. For a detailed explanation of each access to their source code, refer to the official [hugin](http://hugin.sourceforge.net/docs/) and [exiftool](https://exiftool.org/), as well as PetteriAimonen's [focus-stack](https://github.com/PetteriAimonen/focus-stack) documentation. For Windows, we provide a set of precompiled executable files of the required applications in **/external**.
...
<p align="center" width="100%">
  <img width = "65%" src="images/ProjectSettingsWin.png" />
</p>
...
Within the directory of the downloaded Meshroom installation, go to the following folder and check the file “**cameraSensors.db**” to see if your camera is listed:
...
```
Once these details have been checked, launch **Meshroom** and drag your images named *…cutout.tif* into **Meshroom**. If the metadata and added camera sensor are recognised, a **green aperture icon** should be displayed over all images.

<p align="center" width="100%">
  <img width = "65%" src="images/meshroom_correctly_loaded.png" />
</p>
